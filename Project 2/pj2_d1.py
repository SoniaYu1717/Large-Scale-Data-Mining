...
# Simplified SVD & generate matrix vectors
s1 = s[0:50]
U1 = U[:,0:50]
V1 = V[0:50,:]
...
for i in range(0,8):
	label_tr[len(data[3])*i : len(data[3])*(i+1)] = i * np.ones([len(data[3]),1])
...
np.savetxt('train_out.csv', tr_out, delimiter = ',', fmt = '%.8f')
np.savetxt('test_out.csv', ts_out, delimiter = ',', fmt = '%.8f')
