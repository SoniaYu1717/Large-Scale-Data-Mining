...
for i = 1:size(user_id)
    W(user_id(i), item_id(i)) = rating(i);
    R(user_id(i), item_id(i)) = 1;
end
...
fprintf('For k = 10, when lambda is 1, the total least squared error is: %f \n', error_1);
...
fprintf('For k = 50, when lambda is 1, the total least squared error is %f\n', error_2);
...
fprintf('For k = 100, when lambda is 1,the total least squared error is: %f\n', error_3);
